...
export class MemosClient {
  constructor(config) {
    if (!config || !config.apiUrl || !config.agentId) {
      throw new Error("MemosClient requires apiUrl and agentId");
    }
  ...
  async request(path, init = {}) {
    ...
    if (!response.ok) {
      throw new Error(data.error || `Memos request failed: ${response.status}`);
    }
  ...
  /**
   * Get the Memos platform status (0G connectivity, manifest, stats).
...
export function MemosPlugin(config) {
  const client = new MemosClient(config);

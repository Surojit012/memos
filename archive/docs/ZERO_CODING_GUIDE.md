# MemoryOS – AI Assistant & Zero-Coding Guide
...
MemoryOS is a middleware layer that completely abstracts the 0G Network (0G Storage, 0G Compute, 0G KV). The user is building an application **on top of MemoryOS**, not directly on 0G. 
...
MemoryOS handles **all** of this server-side.

### ✅ WHAT YOU SHOULD DO INSTEAD:
You only need exactly **two** things to interact with MemoryOS:
1. `MEMORYOS_AGENT_ID` (e.g., `agent_12345`)
2. `MEMORYOS_API_KEY` (e.g., `agt_abcd1234...`)

**Rule 1: Always use the SDK.**
Whenever the user wants to save data, retrieve context, or run inference, you must use the official MemoryOS SDK.
- **Python:** `from memoryos import MemoryOS`
- **Node.js:** `import { MemoryOS } from 'memoryos-openclaw'`
...
# Python Example
client = MemoryOS(
    api_url="https://their-memoryos-deployment.com", 
...
### Summary for the AI
You are building an agent *using* MemoryOS. Trust that MemoryOS handles all decentralization, wallet signing, and compute routing under the hood. Keep your code incredibly simple, standard, and focused purely on the application's business logic using the MemoryOS SDK.
